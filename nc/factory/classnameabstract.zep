...
abstract class ClassNameAbstract extends FactoryAbstract
{
    ...
    public function setArgsByArray(array args) -> void
    {
    ...
    public function setLazyArg(long index, var lazyArg) -> void
    {
    ...
    protected function processLazyArgs() -> void
    {
        ...
        if count(this->lazyArgs) < 1 {
            return;
        }

        for index, lazyArg in this->lazyArgs {
            if isset this->args[index] && is_callable(lazyArg) {
