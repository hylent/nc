...
PHP_METHOD(Nc_Loader_LoaderAbstract, isLoaded) {
	...
	ZEPHIR_SINIT_VAR(_2);
	ZVAL_BOOL(&_2, (x ? 1 : 0));
	ZEPHIR_CALL_FUNCTION(&_3, "trait_exists", NULL, 25, name, &_2);
...
PHP_METHOD(Nc_Loader_LoaderAbstract, register) {
	...
	ZEPHIR_RETURN_CALL_FUNCTION("spl_autoload_register", NULL, 26, this_ptr);
	zephir_check_call_status();
	RETURN_MM();
...
PHP_METHOD(Nc_Loader_LoaderAbstract, unregister) {
	...
	ZEPHIR_RETURN_CALL_FUNCTION("spl_autoload_unregister", NULL, 27, this_ptr);
	zephir_check_call_status();
	RETURN_MM();
