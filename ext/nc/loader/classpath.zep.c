...
ZEPHIR_INIT_CLASS(Nc_Loader_ClassPath) {
	...
	ZEPHIR_REGISTER_CLASS_EX(Nc\\Loader, ClassPath, nc, loader_classpath, nc_loader_loaderadapter_ce, nc_loader_classpath_method_entry, 0);
...
PHP_METHOD(Nc_Loader_ClassPath, __invoke) {
	...
	zval *className_param = NULL, *path, *_0, *_1 = NULL, *_2, *_3, *_4;
	zval *className = NULL;
	...
	if (unlikely(Z_TYPE_P(path) != IS_STRING)) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(nc_loader_exception_ce, "Invalid class path option", "nc/loader/classpath.zep", 21);
		return;
	}
	...
	if (unlikely(!(zephir_class_exists(className, zephir_is_true(ZEPHIR_GLOBAL(global_false))  TSRMLS_CC)))) {
		...
		ZEPHIR_INIT_VAR(_4);
		ZEPHIR_CONCAT_SVSVS(_4, "Cannot find class `", className, "` in path `", path, "`");
