...
PHP_METHOD(Nc_Loader_ClassPath, __invoke) {
	...
	if (unlikely(Z_TYPE_P(path) != IS_STRING)) {
		...
		zephir_throw_exception_debug(_3$$4, "nc/loader/classpath.zep", 21 TSRMLS_CC);
		ZEPHIR_MM_RESTORE();
		return;
	}
