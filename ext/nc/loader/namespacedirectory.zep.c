...
ZEPHIR_INIT_CLASS(Nc_Loader_NamespaceDirectory) {
	...
	ZEPHIR_REGISTER_CLASS_EX(Nc\\Loader, NamespaceDirectory, nc, loader_namespacedirectory, nc_loader_loaderadapter_ce, nc_loader_namespacedirectory_method_entry, 0);
...
PHP_METHOD(Nc_Loader_NamespaceDirectory, __invoke) {
	...
	zval *className_param = NULL, *match = NULL, *pos = NULL, *arr = NULL, *_0 = NULL, _1 = zval_used_for_init, *_4 = NULL, *_6, *_8, *_9, *_10, _11, *_12, *_13;
	zval *className = NULL, *path = NULL, *_7 = NULL, *_14;
	...
	if (unlikely(_3)) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(nc_loader_exception_ce, "Invalid namespace directory options", "nc/loader/namespacedirectory.zep", 44);
		return;
	}
	ZEPHIR_SINIT_NVAR(_1);
	...
	zephir_array_fetch_long(&_8, arr, 1, PH_NOISY | PH_READONLY, "nc/loader/namespacedirectory.zep", 48 TSRMLS_CC);
	if (zephir_is_true(_8)) {
		ZEPHIR_INIT_NVAR(_0);
		zephir_fast_strtolower(_0, path);
		zephir_get_strval(path, _0);
	}
	zephir_array_fetch_long(&_9, arr, 0, PH_NOISY | PH_READONLY, "nc/loader/namespacedirectory.zep", 51 TSRMLS_CC);
	ZEPHIR_INIT_VAR(_10);
	ZEPHIR_SINIT_NVAR(_1);
	ZVAL_STRING(&_1, "\\", 0);
	ZEPHIR_SINIT_VAR(_11);
	ZVAL_STRING(&_11, "/", 0);
	zephir_fast_str_replace(_10, &_1, &_11, path TSRMLS_CC);
	ZEPHIR_INIT_VAR(_12);
	ZEPHIR_CONCAT_VSVS(_12, _9, "/", _10, ".php");
	zephir_get_strval(path, _12);
	...
	if (unlikely(!(zephir_class_exists(className, zephir_is_true(ZEPHIR_GLOBAL(global_false))  TSRMLS_CC)))) {
		...
		ZEPHIR_INIT_VAR(_14);
		ZEPHIR_CONCAT_SVSVS(_14, "Cannot find class `", className, "` in path `", path, "`");
