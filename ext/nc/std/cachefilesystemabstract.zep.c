...
ZEPHIR_INIT_CLASS(Nc_Std_CacheFileSystemAbstract) {
	...
	ZEPHIR_REGISTER_CLASS(Nc\\Std, CacheFileSystemAbstract, nc, std_cachefilesystemabstract, nc_std_cachefilesystemabstract_method_entry, ZEND_ACC_EXPLICIT_ABSTRACT_CLASS);

	zend_declare_property_null(nc_std_cachefilesystemabstract_ce, SL("baseDirectory"), ZEND_ACC_PROTECTED TSRMLS_CC);

	zend_class_implements(nc_std_cachefilesystemabstract_ce TSRMLS_CC, 1, nc_std_cacheinterface_ce);
	return SUCCESS;
...
PHP_METHOD(Nc_Std_CacheFileSystemAbstract, setCache) {
	...
	ZEPHIR_SINIT_VAR(_4);
	ZVAL_LONG(&_4, 2);
	ZEPHIR_CALL_FUNCTION(&_5, "file_put_contents", NULL, 29, path, _3, &_4);
	zephir_check_call_status();
	if (unlikely(!zephir_is_true(_5))) {
		...
		zephir_check_call_status();
		zephir_throw_exception_debug(_6$$3, "nc/std/cachefilesystemabstract.zep", 19 TSRMLS_CC);
		ZEPHIR_MM_RESTORE();
		return;
	}
	ZEPHIR_MM_RESTORE();
...
PHP_METHOD(Nc_Std_CacheFileSystemAbstract, deleteCache) {
	...
	if (_2) {
		ZEPHIR_CALL_FUNCTION(&_3, "unlink", NULL, 30, path);
		zephir_check_call_status();
	...
	if (unlikely(_2)) {
		...
		zephir_check_call_status();
		zephir_throw_exception_debug(_4$$3, "nc/std/cachefilesystemabstract.zep", 39 TSRMLS_CC);
		ZEPHIR_MM_RESTORE();
		return;
	}
	ZEPHIR_MM_RESTORE();
...
PHP_METHOD(Nc_Std_CacheFileSystemAbstract, path) {
	...
	ZEPHIR_CALL_FUNCTION(&_0, "sha1", NULL, 31, identifier);
	zephir_check_call_status();
	zephir_get_strval(_1, _0);
	...
	if (mkdirIfNeeded) {
		ZEPHIR_CALL_FUNCTION(&_6$$3, "dirname", NULL, 32, path);
		zephir_check_call_status();
		...
		if (_8$$3) {
			...
			ZVAL_LONG(&_9$$3, 0755);
			ZEPHIR_CALL_FUNCTION(&_10$$3, "mkdir", NULL, 33, dirname, &_9$$3, ZEPHIR_GLOBAL(global_true));
			zephir_check_call_status();
		...
		if (unlikely(_8$$3)) {
			...
			zephir_check_call_status();
			zephir_throw_exception_debug(_11$$4, "nc/std/cachefilesystemabstract.zep", 62 TSRMLS_CC);
			ZEPHIR_MM_RESTORE();
			return;
		}
	}
