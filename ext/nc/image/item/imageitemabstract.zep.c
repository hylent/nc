...
PHP_METHOD(Nc_Image_Item_ImageItemAbstract, __construct) {
	...
	if (!(zephir_instance_of_ev(backend, nc_image_imageinterface_ce TSRMLS_CC))) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(spl_ce_InvalidArgumentException, "Parameter 'backend' must be an instance of 'Nc\\Image\\ImageInterface'", "", 0);
		return;
	}
