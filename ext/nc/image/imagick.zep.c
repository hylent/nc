...
PHP_METHOD(Nc_Image_Imagick, __construct) {
	...
	ZEPHIR_CALL_PARENT(NULL, nc_image_imagick_ce, this_ptr, "__construct", &_2, 54, defaultOptions);
	zephir_check_call_status();
	ZEPHIR_MM_RESTORE();
...
PHP_METHOD(Nc_Image_Imagick, fromPath) {
	...
	if (!(!extension) && Z_STRLEN_P(extension)) {
		...
		ZVAL_LONG(&_6$$4, 4);
		ZEPHIR_CALL_FUNCTION(&_7$$4, "pathinfo", NULL, 33, path, &_6$$4);
		zephir_check_call_status();
...
PHP_METHOD(Nc_Image_Imagick, save) {
	...
	if (zephir_fast_strlen_ev(destPath) > 0) {
		...
		ZEPHIR_OBS_VAR(_2$$4);
		zephir_read_property(&_2$$4, im, SL("handler"), PH_NOISY_CC);
