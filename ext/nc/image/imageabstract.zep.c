...
PHP_METHOD(Nc_Image_ImageAbstract, validTextOptions) {
	...
	if (zephir_array_isset_string_fetch(&v, merged, SS("color"), 0 TSRMLS_CC)) {
		...
		if (_13) {
			...
			zephir_array_fetch_long(&_16, m, 1, PH_NOISY | PH_READONLY, "nc/image/imageabstract.zep", 59 TSRMLS_CC);
			...
			zephir_array_fetch_long(&_19, m, 2, PH_NOISY | PH_READONLY, "nc/image/imageabstract.zep", 60 TSRMLS_CC);
			...
			zephir_array_fetch_long(&_22, m, 3, PH_NOISY | PH_READONLY, "nc/image/imageabstract.zep", 61 TSRMLS_CC);
