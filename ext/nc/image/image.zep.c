...
ZEPHIR_INIT_CLASS(Nc_Image_Image) {
	...
	ZEPHIR_REGISTER_CLASS_EX(Nc\\Image, Image, nc, image_image, nc_image_item_ce, nc_image_image_method_entry, 0);
...
PHP_METHOD(Nc_Image_Image, draw) {
	...
	if (!(zephir_instance_of_ev(srcItem, nc_image_item_ce TSRMLS_CC))) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(spl_ce_InvalidArgumentException, "Parameter 'srcItem' must be an instance of 'Nc\\Image\\Item'", "", 0);
		return;
	}
	_0 = zephir_fetch_nproperty_this(this_ptr, SL("backend"), PH_NOISY_CC);
	ZEPHIR_INIT_VAR(_1);
...
PHP_METHOD(Nc_Image_Image, drawTo) {
	...
	do {
		...
		ZEPHIR_INIT_VAR(_8);
		object_init_ex(_8, nc_image_exception_ce);
		ZEPHIR_SINIT_VAR(_9);
		...
		zephir_check_call_status();
		zephir_throw_exception_debug(_8, "nc/image/image.zep", 75 TSRMLS_CC);
		ZEPHIR_MM_RESTORE();
		return;
	} while(0);
