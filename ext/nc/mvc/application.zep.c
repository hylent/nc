...
PHP_METHOD(Nc_Mvc_Application, __construct) {
	...
	if (zephir_fast_count_int(&a TSRMLS_CC) > 1) {
		...
		ZEPHIR_MAKE_REF(&a);
		ZEPHIR_CALL_FUNCTION(NULL, "uksort", NULL, 100, &a, &_5$$4);
		ZEPHIR_UNREF(&a);
	...
	ZEPHIR_CALL_FUNCTION(NULL, "spl_autoload_register", NULL, 101, &_6, &__$false, &__$true);
	zephir_check_call_status();
	ZEPHIR_INIT_NVAR(&a);
	array_init(&a);
	...
	ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&routingRules), _9, _10, _8)
	{
		...
		ZVAL_STRING(&_11$$5, "/");
		ZEPHIR_CALL_FUNCTION(&_12$$5, "substr_count", &_13, 102, &k, &_11$$5);
		zephir_check_call_status();
	...
	if (zephir_fast_count_int(&a TSRMLS_CC) > 1) {
		ZVAL_LONG(&_14$$6, 1);
		ZEPHIR_MAKE_REF(&a);
		ZEPHIR_CALL_FUNCTION(NULL, "krsort", NULL, 103, &a, &_14$$6);
		ZEPHIR_UNREF(&a);
		zephir_check_call_status();
	}
...
PHP_METHOD(Nc_Mvc_Application, __destruct) {
	...
	ZEPHIR_CALL_FUNCTION(NULL, "spl_autoload_unregister", NULL, 104, &_0);
	zephir_check_call_status();
	ZEPHIR_MM_RESTORE();
	...
	if (zephir_fast_count_int(&_0 TSRMLS_CC) > 0) {
		...
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&_2$$3), _4$$3, _5$$3, _3$$3)
		{
			...
			if (ZEPHIR_IS_LONG_IDENTICAL(&_6$$4, 0)) {
				...
				ZVAL_STRING(&_10$$5, "/");
				ZEPHIR_CALL_FUNCTION(&_11$$5, "strtr", &_12, 97, &_8$$5, &_9$$5, &_10$$5);
				zephir_check_call_status();
...
PHP_METHOD(Nc_Mvc_Application, __invoke) {
	...
	if (Z_TYPE_P(context) == IS_NULL) {
		...
		if (ZEPHIR_IS_STRING(&_0$$3, "cli")) {
			...
			ZEPHIR_CALL_METHOD(NULL, context, "__construct", NULL, 105);
			zephir_check_call_status();
		} else {
			object_init_ex(context, nc_mvc_contexthttp_ce);
			ZEPHIR_CALL_METHOD(NULL, context, "__construct", NULL, 106);
			zephir_check_call_status();
		}
	}
	...
	ZEPHIR_CALL_METHOD(&_4, &injector, "__invoke", NULL, 107, &controllerClass, &_5);
	zephir_check_call_status();
...
PHP_METHOD(Nc_Mvc_Application, onSwooleRequest) {
	...
	int ZEPHIR_LAST_CALL_STATUS;
	zval *req, req_sub, *resp, resp_sub, ex, _0$$3, _1$$4, _2$$4, _3$$4, _4$$5, _5$$7, _6$$7, _7$$7;
	ZEPHIR_INIT_THIS();
	...
	/* try_start_1: */
		...
		ZEPHIR_INIT_VAR(&_0$$3);
		object_init_ex(&_0$$3, nc_mvc_contexthttpswoole_ce);
		ZEPHIR_CALL_METHOD(NULL, &_0$$3, "__construct", NULL, 108, req, resp);
		zephir_check_call_status_or_jump(try_end_1);
		ZEPHIR_CALL_METHOD(NULL, this_ptr, "__invoke", NULL, 0, &_0$$3);
		zephir_check_call_status_or_jump(try_end_1);
	...
	/* try_start_2: */
		...
		ZEPHIR_CALL_FUNCTION(&_4$$5, "session_status", NULL, 109);
		zephir_check_call_status_or_jump(try_end_2);
		if (ZEPHIR_IS_LONG(&_4$$5, 2)) {
	...
	if (EG(exception)) {
		...
		if (zephir_instance_of_ev(&ex, zend_exception_get_default(TSRMLS_C) TSRMLS_CC)) {
			...
			ZEPHIR_GET_CONSTANT(&_5$$7, "PHP_EOL");
			ZEPHIR_INIT_VAR(&_6$$7);
			ZEPHIR_CONCAT_VV(&_6$$7, &ex, &_5$$7);
			ZEPHIR_INIT_VAR(&_7$$7);
			ZVAL_STRING(&_7$$7, "php://stderr");
			zephir_file_put_contents(NULL, &_7$$7, &_6$$7 TSRMLS_CC);
		}
	}
...
PHP_METHOD(Nc_Mvc_Application, route) {
	...
	if (zephir_fast_count_int(&_0 TSRMLS_CC) > 0) {
		...
		zephir_is_iterable(&_1$$3, 0, "nc/mvc/application.zep", 138);
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&_1$$3), _3$$3, _4$$3, _2$$3)
		{
			...
			zephir_is_iterable(&routingNames, 0, "nc/mvc/application.zep", 137);
