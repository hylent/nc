...
PHP_METHOD(Nc_Db_PdoMysql, insert) {
	...
	array_init(vs);
	ZEPHIR_INIT_VAR(_0);
	zephir_is_iterable(data, &_2, &_1, 0, 0, "nc/db/pdomysql.zep", 16);
	for (
	  ...
	  ; zephir_hash_move_forward_ex(_2, &_1)
		...
		zephir_array_append(&ks, k, PH_SEPARATE, "nc/db/pdomysql.zep", 12);
		ZEPHIR_INIT_LNVAR(_4$$3);
		ZEPHIR_CONCAT_SV(_4$$3, ":", k);
		zephir_array_append(&vs, _4$$3, PH_SEPARATE, "nc/db/pdomysql.zep", 13);
	}
	if (upsert) {
		ZEPHIR_INIT_VAR(s);
...
PHP_METHOD(Nc_Db_PdoMysql, upsert) {
	...
	int ZEPHIR_LAST_CALL_STATUS;
	zval *data = NULL;
	zval *table_param = NULL, *data_param = NULL, *primaryKey = NULL, *k = NULL, *_9, *_10, **_2$$3, *_3$$6 = NULL, *_4$$6 = NULL, *_7$$8, *_8$$8;
	...
	if (Z_TYPE_P(primaryKey) == IS_ARRAY) {
		if (unlikely(!zephir_is_true(primaryKey))) {
			ZEPHIR_THROW_EXCEPTION_DEBUG_STR(nc_db_exception_ce, "Cannot upsert with empty where", "nc/db/pdomysql.zep", 43);
			return;
		}
		zephir_is_iterable(primaryKey, &_1$$3, &_0$$3, 0, 0, "nc/db/pdomysql.zep", 50);
		for (
		  ...
		  ; zephir_hash_move_forward_ex(_1$$3, &_0$$3)
			...
			if (unlikely(!(zephir_array_isset(data, k)))) {
				...
				zephir_check_call_status();
				zephir_throw_exception_debug(_3$$6, "nc/db/pdomysql.zep", 47 TSRMLS_CC);
				ZEPHIR_MM_RESTORE();
				return;
			}
		}
		...
		if (unlikely(!(zephir_array_isset(data, k)))) {
			...
			zephir_check_call_status();
			zephir_throw_exception_debug(_7$$8, "nc/db/pdomysql.zep", 53 TSRMLS_CC);
			ZEPHIR_MM_RESTORE();
			return;
		}
	}
...
PHP_METHOD(Nc_Db_PdoMysql, countAndSelect) {
	...
	ZEPHIR_CALL_FUNCTION(&_4, "preg_replace", NULL, 47, _2, _3, s);
