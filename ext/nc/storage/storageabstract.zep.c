...
ZEPHIR_INIT_CLASS(Nc_Storage_StorageAbstract) {
	...
	zend_declare_class_constant_long(nc_storage_storageabstract_ce, SL("COPY"), 0 TSRMLS_CC);

	zend_declare_class_constant_long(nc_storage_storageabstract_ce, SL("MOVE"), 1 TSRMLS_CC);

	zend_declare_class_constant_long(nc_storage_storageabstract_ce, SL("MOVE_UPLOADED_FILE"), 2 TSRMLS_CC);
...
PHP_METHOD(Nc_Storage_StorageAbstract, generateUri) {
	...
	if (zephir_is_true(_0)) {
		...
		ZEPHIR_RETURN_CALL_FUNCTION("call_user_func", NULL, 15, _1$$3, source, prefix, extension);
	...
	if (zephir_fast_strlen_ev(prefix) > 0) {
		...
		if (unlikely(!zephir_is_true(_4$$4))) {
			...
			zephir_check_call_status();
			zephir_throw_exception_debug(_6$$5, "nc/storage/storageabstract.zep", 33 TSRMLS_CC);
			ZEPHIR_MM_RESTORE();
			return;
		}
	...
	ZEPHIR_CALL_CE_STATIC(&_8, nc_std_ce, "uuid", &_9, 16, _10);
	zephir_check_call_status();
