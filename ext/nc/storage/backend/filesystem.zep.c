...
ZEPHIR_INIT_CLASS(Nc_Storage_Backend_FileSystem) {
	...
	ZEPHIR_REGISTER_CLASS_EX(Nc\\Storage\\Backend, FileSystem, nc, storage_backend_filesystem, nc_storage_backend_storagebackendadapter_ce, nc_storage_backend_filesystem_method_entry, 0);

	zend_declare_property_null(nc_storage_backend_filesystem_ce, SL("baseDirectory"), ZEND_ACC_PROTECTED TSRMLS_CC);
...
PHP_METHOD(Nc_Storage_Backend_FileSystem, store) {
	...
	zend_bool mv, _7;
	zval *source_param = NULL, *group_param = NULL, *extension_param = NULL, *mv_param = NULL, *temp = NULL, *success = NULL, *_0 = NULL, *_2, *_3, *_5 = NULL, _8, *_9 = NULL, *_11, *_12;
	...
	if (unlikely(_7)) {
		ZEPHIR_INIT_VAR(_11);
		object_init_ex(_11, nc_storage_backend_exception_ce);
		ZEPHIR_INIT_VAR(_12);
		ZEPHIR_CONCAT_SV(_12, "Cannot mkdir: ", temp);
		ZEPHIR_CALL_METHOD(NULL, _11, "__construct", NULL, _12);
		zephir_check_call_status();
		zephir_throw_exception_debug(_11, "nc/storage/backend/filesystem.zep", 22 TSRMLS_CC);
		ZEPHIR_MM_RESTORE();
		return;
	}
	if (mv) {
