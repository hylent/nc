...
PHP_METHOD(Nc_Storage_FileSystem, store) {
	...
	long flag;
	zval *source_param = NULL, *prefix_param = NULL, *extension_param = NULL, *flag_param = NULL, *temp = NULL, *success = NULL, *_0 = NULL, *_2, *_3, *_4 = NULL, _6, *_7 = NULL, *_8$$3, *_9$$3, *_10$$7;
	zval *source = NULL, *prefix = NULL, *extension = NULL, *destUri = NULL, *destPath = NULL, *_1 = NULL, *_11$$7;
	...
	do {
		if (flag == 2) {
			ZEPHIR_CALL_FUNCTION(&success, "move_uploaded_file", NULL, 128, source, destPath);
			zephir_check_call_status();
			break;
		}
		if (flag == 1) {
			ZEPHIR_CALL_FUNCTION(&success, "rename", NULL, 129, source, destPath);
			zephir_check_call_status();
			break;
		}
		ZEPHIR_CALL_FUNCTION(&success, "copy", NULL, 130, source, destPath);
		zephir_check_call_status();
		break;
	} while(0);
	...
	if (unlikely(!zephir_is_true(success))) {
		...
		ZEPHIR_CALL_METHOD(NULL, _10$$7, "__construct", NULL, 2, _11$$7);
		zephir_check_call_status();
		zephir_throw_exception_debug(_10$$7, "nc/storage/filesystem.zep", 40 TSRMLS_CC);
		ZEPHIR_MM_RESTORE();
		return;
	}
	RETURN_CTOR(destUri);
...
PHP_METHOD(Nc_Storage_FileSystem, remove) {
	...
	if ((zephir_file_exists(path TSRMLS_CC) == SUCCESS)) {
		ZEPHIR_RETURN_CALL_FUNCTION("unlink", NULL, 115, path);
		zephir_check_call_status();
		RETURN_MM();
	}
	RETURN_MM_BOOL(0);
