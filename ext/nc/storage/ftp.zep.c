...
PHP_METHOD(Nc_Storage_Ftp, __construct) {
	...
	zend_bool ssl, _4;
	long port, timeout;
	zval *host_param = NULL, *user_param = NULL, *passwd_param = NULL, *baseDirectory_param = NULL, *port_param = NULL, *timeout_param = NULL, *ssl_param = NULL, *ftp = NULL, _0 = zval_used_for_init, _1 = zval_used_for_init, *_2 = NULL, *_5 = NULL, *_6 = NULL;
	zval *host = NULL, *user = NULL, *passwd = NULL, *baseDirectory = NULL, *_3 = NULL;
	...
	if (ssl) {
		...
		ZVAL_LONG(&_1, timeout);
		ZEPHIR_CALL_FUNCTION(&ftp, "ftp_connect", NULL, host, &_0, &_1);
		zephir_check_call_status();
	}
	if (unlikely(!zephir_is_true(ftp))) {
		ZEPHIR_INIT_VAR(_2);
		object_init_ex(_2, nc_storage_exception_ce);
		ZEPHIR_INIT_VAR(_3);
		ZEPHIR_CONCAT_SV(_3, "Cannot connect to ftp server: ", host);
		ZEPHIR_CALL_METHOD(NULL, _2, "__construct", NULL, _3);
		zephir_check_call_status();
		zephir_throw_exception_debug(_2, "nc/storage/ftp.zep", 26 TSRMLS_CC);
		ZEPHIR_MM_RESTORE();
		return;
	}
	_4 = zephir_is_true(user);
	if (_4) {
		ZEPHIR_CALL_FUNCTION(&_5, "ftp_login", NULL, ftp, user, passwd);
		zephir_check_call_status();
	...
	if (unlikely(_4)) {
		...
		ZEPHIR_CONCAT_SVSV(_3, "Cannot login ftp server: ", user, "@", host);
		ZEPHIR_CALL_METHOD(NULL, _2, "__construct", NULL, _3);
		zephir_check_call_status();
		zephir_throw_exception_debug(_2, "nc/storage/ftp.zep", 30 TSRMLS_CC);
		ZEPHIR_MM_RESTORE();
		return;
	}
	ZEPHIR_CALL_FUNCTION(&_6, "ftp_pasv", NULL, ftp, ZEPHIR_GLOBAL(global_true));
	zephir_check_call_status();
...
PHP_METHOD(Nc_Storage_Ftp, store) {
	...
	int ZEPHIR_LAST_CALL_STATUS;
	zend_bool rm;
	zval *source_param = NULL, *group_param = NULL, *extension_param = NULL, *rm_param = NULL, *_0 = NULL, *_2, *_3, *_4 = NULL, *_6, _7, *_8 = NULL;
...
PHP_METHOD(Nc_Storage_Ftp, mkDirIfNotExists) {
	...
	zephir_check_call_status();
	zephir_is_iterable(stack, &_12, &_11, 0, 1, "nc/storage/ftp.zep", 93);
	for (
