...
PHP_METHOD(Nc_Storage_Fastdfs, store) {
	...
	int ZEPHIR_LAST_CALL_STATUS;
	long flag;
	zval *source_param = NULL, *prefix_param = NULL, *extension_param = NULL, *flag_param = NULL, *extensionArg = NULL, *groupArg = NULL, *destUri = NULL, *_0 = NULL, *_1, *_2;
	...
	_1 = zephir_fetch_nproperty_this(this_ptr, SL("prefixGroups"), PH_NOISY_CC);
	if (!(zephir_array_isset_fetch(&groupArg, _1, prefix, 0 TSRMLS_CC))) {
		ZEPHIR_INIT_NVAR(groupArg);
		ZVAL_NULL(groupArg);
	}
	ZEPHIR_INIT_VAR(_2);
	array_init(_2);
	ZEPHIR_CALL_ZVAL_FUNCTION(&destUri, fun, NULL, 0, source, extensionArg, _2, groupArg);
	zephir_check_call_status();
